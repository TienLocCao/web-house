...
export async function PATCH(
  ...
  return withAdminAuth(request, async (admin) => {
    ...
    try {
      ...
      if (validated.slug) {
        const [conflict] =
          await sql`SELECT id FROM products WHERE slug = ${validated.slug} AND id != ${productId}`;
        if (conflict) {
      ...
      UPDATE products SET
        ...
        gallery = COALESCE(${
          validated.gallery ? JSON.stringify(validated.gallery) : null
        }, gallery),
        ...
        dimensions = COALESCE(${
          validated.dimensions ? JSON.stringify(validated.dimensions) : null
        }, dimensions),
    ...
    `;
      ...
      if (error instanceof z.ZodError) {
        ...
        return NextResponse.json(
          { message: "Validation failed", errors: issues },
          { status: 400 }
        );
      }
      console.error(error);
